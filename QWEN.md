# TiendaApp - QWEN Context

## Configuración de Idioma

El asistente debe comunicarse siempre en español. Para mantener la consistencia en todos los intercambios, se utilizará el idioma español como idioma principal de comunicación.

## Project Overview

TiendaApp is a WPF desktop application built with .NET 9.0 for managing a store (users, articles, sales). It uses a layered architecture with dependency injection and SQLite database storage. The application implements a clean architecture with clear separation of concerns and follows modern .NET development practices.
...
- **Solution File**: `TiendaApp.sln` contains all projects
- **Source Code**: Located in `src/` directory
- **Tests**: Located in `tests/` directory
- **Configuration**: Project setup documented in README.md and WARP.md

## Configuración de formato de archivos

Todos los archivos de texto deben guardarse con final de línea en formato CRLF (Carriage Return + Line Feed) en lugar de LF (Line Feed) para mantener compatibilidad con el sistema operativo Windows.